...
async function analyzeOrders() {
    try {
        ...
        // // Metrics for Part 2
        // Create a map to track unique elements in inputs and outputs
        ...
        // WETH
        const wethInputs = filteredOrders
            .flatMap(order => order.inputs)
            .filter(input => input.token.symbol === 'WETH')
            .reduce((sum, input) => {
                const uniqueKey = input.id;
                uniqueEntries.add(uniqueKey); // Track all inputs
                return sum.add(ethers.BigNumber.from(input.balance));
        ...
        const wethOutputs = filteredOrders
            .flatMap(order => order.outputs)
            .filter(output => output.token.symbol === 'WETH')
            .reduce((sum, output) => {
                const uniqueKey = output.id;
                if (!uniqueEntries.has(uniqueKey)) { // Only add if it's not already counted in inputs
                    uniqueEntries.add(uniqueKey);
        ...
        const totalWETH = ethers.utils.formatUnits(wethInputs.add(wethOutputs), 18);
        console.log('Total WETH:', totalWETH);

        // USDC/USDT
        const usdcUsdtInputs = filteredOrders
            .flatMap(order => order.inputs)
            .filter(input => input.token.symbol === 'USDC' || input.token.symbol === 'USDT')
            .reduce((sum, input) => {
                const uniqueKey = input.id;
                uniqueEntries.add(uniqueKey); // Track all inputs
                return sum.add(ethers.BigNumber.from(input.balance));
        ...
        const usdcUsdtOutputs = filteredOrders
            .flatMap(order => order.outputs)
            .filter(output => output.token.symbol === 'USDC' || output.token.symbol === 'USDT')
            .reduce((sum, output) => {
                const uniqueKey = output.id;
                if (!uniqueEntries.has(uniqueKey)) { // Only add if it's not already counted in inputs
                    uniqueEntries.add(uniqueKey);
        ...
        const totalUSDCUSDT = ethers.utils.formatUnits(usdcUsdtInputs.add(usdcUsdtOutputs), 6); // Assuming USDC/USDT have 6 decimals
        console.log('Total USDC/USDT:', totalUSDCUSDT);
        ...
        processOrdersWithAggregation(filteredOrders);
...
async function fetchTrades(orderHash: string): Promise<any[]> {
    try {
      const response = await axios.post(endpoint, {
        query: tradeQuery,
        variables: { orderHash },
...
async function processOrdersWithAggregation(filteredOrders: any[]) {
    ...
    for (const order of filteredOrders) {
      ...
      try {
        ...
        const trades = await fetchTrades(orderHash);
