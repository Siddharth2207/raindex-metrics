...
import { tokenConfig, networkConfig, allowedNetworks, allowedTokens, allowedDurations } from './config';
import {orderMetrics, tokenMetrics, calculateCombinedVaultBalance} from './metrics/orderAndTokenMetreics'
...
async function generateReportForToken(token: string, network: string, duration: string) {
  try {
    ...
    const totalRaindexVolumeUsd = Number(
      aggregatedResults?.find((e: any) => e.address.toLowerCase() === tokenAddress)?.totalVolumeForDurationUsd || 0
    );    

    // Generate insights
    const totalExternalTrades = totalTokenExternalTradesForDuration - totalRaindexTradesForDuration;
...
cliCommand
  ...
  .option(
    '-t, --token <symbol>',
    `Token symbol. Any of [${allowedTokens.join(', ')}]`
  )
  .option(
    '-n, --network <name>',
    `Network name. Any of [${allowedNetworks.join(', ')}]`
  )
  .option(
    '-d, --duration <duration>',
    `Duration. Any of [${allowedDurations.join(', ')}]`
  );
